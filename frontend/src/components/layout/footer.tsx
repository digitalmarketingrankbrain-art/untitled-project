...
function Footer() {
  ...
  return (
    <footer className="border-t border-slate-800 bg-[#081726] text-slate-300">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-12">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 flex flex-col justify-between">
            ...
            <div className="mt-4 flex items-center gap-2.5 rounded-lg border border-slate-800 bg-[#0b2341] p-3 shadow-inner">
          ...
          <div className="hidden grid-cols-2 md:grid-cols-4 gap-6 lg:col-span-8 sm:grid">
            {FOOTER_GROUPS.map((group) => (
              <div key={group.label} className="flex flex-col">
                <p className="mb-3 text-[11px] font-extrabold uppercase tracking-wider text-amber-400 border-b border-slate-800 pb-2">
                  {group.label}
                </p>
                <ul className="flex flex-col gap-2">
                  {group.links.map((link) => (
                    <li key={link.href + link.label}>
                      <Link
                        href={link.href}
                        className="text-[12px] font-normal text-slate-300 transition-colors hover:text-amber-400 leading-tight block"
                      >
                        {link.label}
                      </Link>
                    </li>
          ...
          {/* Mobile Accordion */}
          <div className="flex flex-col divide-y divide-slate-800 border-y border-slate-800 lg:hidden">
            {FOOTER_GROUPS.map((group) => {
              ...
              return (
                <div key={group.label}>
                  <button
                    ...
                    className="flex w-full items-center justify-between py-3 text-xs font-bold text-slate-200"
                  >
                    <span>{group.label}</span>
                    <ChevronDown
                      className={cn("size-3.5 text-amber-400 transition-transform", isOpen && "rotate-180")}
                    />
                  </button>
                  {isOpen && (
        ...
        {/* Legal & Copyright Bar */}
        <div className="mt-10 flex flex-col items-center justify-between gap-4 border-t border-slate-800/80 pt-6 text-[11px] text-slate-400 sm:flex-row">
          ...
          <div className="flex flex-wrap items-center gap-3 font-medium">
            <Link href="/legal/terms-of-use" className="hover:text-amber-400 transition-colors">Terms of Use</Link>
            ...
            <span className="text-slate-700">|</span>
            <Link href="/legal/disclaimer" className="hover:text-amber-400 transition-colors">Disclaimer</Link>
          </div>
        </div>
      </div>
    </footer>
